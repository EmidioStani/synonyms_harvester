...
def getVerbs(c):
    ...
    print("post_tags %s" % post_tags)
    # https://stackoverflow.com/questions/40167612/how-to-keep-only-the-noun-words-in-a-wordlist-python-nltk
    verbs = list(set([word for word,pos in post_tags if (pos == 'VB' or pos == 'VBD' or pos == 'VBN' or pos == 'VBG')]))
    print("verbs %s" % verbs)
    return verbs

def checkLemmas(nouns, wik_file):
    x= []
    for noun in nouns:
...
def checkVerbsLemmas(verbs, wik_file):
    x= []
    for verb in verbs:
        ...
        for a, b, c in sorted(g.triples((f, SKOSXL.literalForm , None))):
            ...
            if(len(mylist) < 10):
                ...
                if len(words) < 5:
                    ...
                    for word in words:
                        print("Searching for %s exact match..." % word)
                        list2 = checkExactMatch(c,WIKTIONARY_FILE)
                        mylist.extend(list2)
            mylist = list(set(mylist))
            ...
            if(len(mylist) < 15):
                print("Checking noun with %s lemmas..." % lenNouns)
                list2 = checkLemmas(nouns,WIKTIONARY_FILE)
                mylist.extend(list2)
            mylist = list(set(mylist))
            ...
            if(len(mylist) < 25):
                print("Checking verbs with %s lemmas..." % lenVerbs)
                list2 = checkVerbsLemmas(verbs,WIKTIONARY_FILE)
                mylist.extend(list2)
            mylist = list(set(mylist))

            if(len(mylist) < 30):
                for verb in verbs:
                    print("Searching verb %s in Altervista..." % verb)
